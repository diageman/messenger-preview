...
export function ChatList({ chats, selectedChatId, onSelectChat, searchQuery, onSearchChange, activeCategory, onCategoryChange, isSearchOpen, onSearchOpenChange, unreadTotal, onDeleteChatForMe, onDeleteChatForAll }: ChatListProps) {
  ...
  const Item = (c: ChatWithPeer) => <ChatListItem key={c.id} chat={c} isSelected={selectedChatId === c.id} onSelect={() => onSelectChat(c.id)} onDeleteChatForMe={onDeleteChatForMe} onDeleteChatForAll={onDeleteChatForAll} />;
  return (<div className="relative flex h-full flex-col bg-bg-panel">
    ...
    <div className="border-b border-border-soft px-2 py-2"><div className="flex gap-1 overflow-x-auto">{(Object.keys(categoryLabels) as ChatCategory[]).map(c => <Button key={c} variant="ghost" size="sm" onClick={() => onCategoryChange(c)} className={cn('whitespace-nowrap text-xs', activeCategory === c ? 'bg-bg-elevated text-text-primary font-medium' : 'text-text-muted')}>{categoryLabels[c]}</Button>)}</div></div>    <ScrollArea className="flex-1"><div className="flex flex-col">
      {pinned.length > 0 && <div className="mb-3"><div className="px-4 py-2"><div className="flex items-center gap-2"><Pin className="h-3.5 w-3.5 text-accent-yellow" /><span className="text-xs font-semibold uppercase tracking-wider text-text-muted">Закреплённые</span></div></div><div className="space-y-0.5"><AnimatePresence initial={false}>{pinned.map(c => Item(c))}</AnimatePresence></div><Separator className="my-2 bg-border-soft" /></div>}
      {recent.length > 0 && <div className="mb-3"><div className="px-4 py-2"><span className="text-xs font-semibold uppercase tracking-wider text-text-muted">Недавние</span></div><div className="space-y-0.5"><AnimatePresence initial={false}>{recent.map(c => Item(c))}</AnimatePresence></div>{other.length > 0 && <Separator className="my-2 bg-border-soft" />}</div>}
      {other.length > 0 && <div className="mb-3"><div className="px-4 py-2"><span className="text-xs font-semibold uppercase tracking-wider text-text-muted">Все чаты</span></div><div className="space-y-0.5"><AnimatePresence initial={false}>{other.map(c => Item(c))}</AnimatePresence></div></div>}
...
}interface ChatListItemProps { chat: ChatWithPeer; isSelected: boolean; onSelect: () => void; onDeleteChatForMe?: (id: string) => void; onDeleteChatForAll?: (id: string) => void; }
function ChatListItem({ chat, isSelected, onSelect, onDeleteChatForMe, onDeleteChatForAll }: ChatListItemProps) {
  ...
  return (
    <motion.div
      ...
      className={cn('group relative flex w-full items-start gap-3 px-3 py-2.5 text-left transition-colors', isSelected ? 'bg-bg-elevated' : 'hover:bg-bg-hover')}>
      ...
      <button onClick={() => { if (!menuOpen) onSelect(); }} className="flex flex-1 items-start gap-3 text-left min-w-0">
        <div className="relative shrink-0">{chat.type === 'direct' ? <Avatar
            size="md"
            src={chat.peerAvatar && (chat.peerAvatar.startsWith('http') || chat.peerAvatar.startsWith('/')) ? chat.peerAvatar : undefined}
            fallback={(!chat.peerAvatar || chat.peerAvatar.startsWith('http') || chat.peerAvatar.startsWith('/')) ? (chat.name?.[0]?.toUpperCase() ?? '?') : chat.peerAvatar}
            status={chat.peerStatus as any}
            showStatus
          /> : <div className="flex h-10 w-10 items-center justify-center rounded-full bg-bg-elevated text-sm font-bold text-text-secondary">G</div>}{chat.isPinned && <Pin className="absolute -right-0.5 -top-0.5 h-3 w-3 text-accent-yellow" />}</div>
